...
Windows タスクスケジューラ
    ...
    └─ 16:05 scripts/execute.py   → 全アクティブ RUN の WAIT 注文を寄付価格で約定判定
...
|---|---|
| 初期資金 | 500,000 円 |
| シミュレーション期間 | 市場 30 日分（config.json で設定） |
| 取引頻度 | 毎営業日 1 回 |
| 対象銘柄 | 日本株大型株 20 銘柄（config.json で設定） |
| ロング集中上限 | 総資産の 30%（単一銘柄） |
| ショート建玉上限 | 合計 250,000 円（全銘柄合算） |
...
### Step 2 — 16:05 （execute.py）

1. アクティブな各 RUN の `pending_orders.json` を読み込む
2. yfinance で当日の**始値（寄付価格）**を取得（全 RUN 共通）
...
```
BUY   注文: 寄付価格 ≤ limit_price → FILLED（安く買いたい）
            寄付価格 > limit_price → UNFILLED（即キャンセル）

SELL  注文: 寄付価格 ≥ limit_price → FILLED（高く売りたい）
            寄付価格 < limit_price → UNFILLED（即キャンセル）

SHORT 注文: 寄付価格 ≥ limit_price → FILLED（高く空売りしたい）
            寄付価格 < limit_price → UNFILLED（即キャンセル）
...
約定価格は指値ではなく**実際の寄付価格**。UNFILLED は即キャンセル。翌営業日に Claude が再判断する。
...
2026-05-26,16:05,7203,BUY,100,2830,FILLED,56700
2026-05-26,08:30,9984,SHORT,100,,WAIT,56700
2026-05-26,16:05,9984,SHORT,100,9180,FILLED,975700
```

| カラム | 説明 |
|---|---|
| date | 取引日（YYYY-MM-DD） |
| time | 08:30 = WAIT 登録、16:05 = 約定処理 |
| symbol | 銘柄コード |
